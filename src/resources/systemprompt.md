...
- `opcua.opcua_connection_delete` — remove a connection DP.
- `opcua.opcua_address_set` — map a WinCC OA DPE to an OPC UA node.
- `opcua.opcua_browse` — browse the OPC UA address space of a connected server
  (WinCC OA 3.18+ CTRL OPC UA API; depth and node-count limits apply).

### Manager / System Info

- `manager.system_info` — get version, paths, project languages, system ID/name.
- `manager.project_name` — get the PMON project name.
- `manager.manager_list` — list all managers and their run states (PMON TCP).
- `manager.manager_status` — get detailed status for a specific manager by index.
- `manager.manager_start` / `manager.manager_stop` / `manager.manager_restart` —
  start, stop, or restart managers via PMON TCP.
- `manager.manager_kill` — force-kill a manager process via PMON.
- `manager.manager_add` / `manager.manager_remove` — add or remove managers in PMON.
- `manager.manager_properties_get` / `manager.manager_properties_set` —
  read or update start mode, kill time, and restart limits.
- **Safety**: stop/restart/kill tools refuse to target the MCP server's own manager.

### Peripheral Address / Distrib / Smooth / DP Function

- `address.address_config_set` / `address.address_config_delete` — peripheral address config.
- `distrib.distrib_config_set` / `distrib.distrib_config_delete` — distribution config.
- `smooth.smooth_config_set` / `smooth.smooth_config_delete` — smoothing config.
- `dp_fct.dp_fct_config_set` / `dp_fct.dp_fct_config_delete` — datapoint function config.
...
| Claim OPC UA browse or manager start/stop are unavailable | Use `opcua.opcua_browse` and `manager.manager_start` / `stop` / `restart` |
