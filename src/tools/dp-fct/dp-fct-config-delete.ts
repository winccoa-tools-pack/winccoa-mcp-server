/**
 * Tool: dp_fct/dp_fct_config_delete
 *
 * Disable DP function on one or more DPEs by setting their config type to DPCONFIG_NONE.
 */
...
export function registerDpFctConfigDelete(server: McpServer): void {
  server.registerTool(
    "dp_fct.dp_fct_config_delete",
    {
      title: "Delete DP Function Configuration",
      description: `Disable the datapoint function configuration on one or more WinCC OA
datapoint elements (DPEs) by setting their dp_fct config type to DPCONFIG_NONE.

Args:
  - dpeNames (string[]): DPE names whose DP function config should be disabled.
  ...
  Each DPE is processed individually so a failure on one does not abort the rest.`,
      inputSchema: {
        dpeNames: z
          ...
          .describe("Datapoint element name(s) to disable DP function on"),
      },
      annotations: {
        readOnlyHint: false,
    ...
    async ({ dpeNames }) => {
      ...
      for (const dpe of dpeNames) {
        try {
          await winccoa.dpSetWait(`${dpe}:_dp_fct.._type`, DPCONFIG_NONE);
