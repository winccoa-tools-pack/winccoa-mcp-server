/**
 * Tool: distrib/distrib_config_delete
 *
 * Disable distribution on one or more DPEs by setting their config type to DPCONFIG_NONE.
 */
...
export function registerDistribConfigDelete(server: McpServer): void {
  server.registerTool(
    ...
    {
      title: "Delete Distribution Configuration",
      description: `Disable distribution on one or more WinCC OA datapoint elements (DPEs)
by setting their distribution config type to DPCONFIG_NONE.

Args:
  - dpeNames (string[]): DPE names whose distribution config should be disabled.
  ...
  Each DPE is processed individually so a failure on one does not abort the rest.`,
      inputSchema: {
        dpeNames: z
          ...
          .describe("Datapoint element name(s) to disable distribution on"),
      },
      annotations: {
        readOnlyHint: false,
    ...
    async ({ dpeNames }) => {
      ...
      for (const dpe of dpeNames) {
        try {
          await winccoa.dpSetWait(`${dpe}:_distrib.._type`, DPCONFIG_NONE);
